...
export class OmniSharpServer {
    ...
    public async autoStart(preferredPath: string): Promise<void> {
        ...
        if (launchTargets.length === 0) {
            await new Promise<void>((resolve, reject) => {
                // 1st watch for files
                const watcher = coc.workspace.createFileSystemWatcher('{**/*.sln,**/*.slnf,**/*.csproj,**/project.json,**/*.csx,**/*.cake}',
    ...
    public async makeRequest<TResponse>(command: string, data?: any, token?: coc.CancellationToken): Promise<TResponse> {
        if (!this.isRunning()) {
