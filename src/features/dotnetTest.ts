...
export default class TestManager extends AbstractProvider {
    ...
    private _getRunSettings(filename: string): string | undefined {
        ...
        const workspaceFolder = vscode.workspace.getWorkspaceFolder(fileUri.toString());

        return path.join(vscode.Uri.parse(workspaceFolder.uri).fsPath, testSettingsPath);
    }
