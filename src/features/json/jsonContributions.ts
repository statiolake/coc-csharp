...
import {
    ...
    CancellationToken, Range, MarkedString, DocumentSelector, languages, workspace, LinesTextDocument
} from 'coc.nvim';
...
export function addJSONProviders(): CompositeDisposable {
    ...
    contributions.forEach(contribution => {
        ...
        subscriptions.add(languages.registerCompletionItemProvider(
            "csharp-json",
            "CSJ",
            selector,
...
export class JSONHoverProvider implements HoverProvider {
    ...
    public provideHover(document: TextDocument, position: Position, token: CancellationToken): Thenable<Hover> {
        ...
        if (node && node.offset <= offset && offset <= node.offset + node.length) {
            let promise = this.jsonContribution.getInfoContribution(vscode.Uri.parse(document.uri).fsPath, location);
            if (promise) {
                return promise.then(htmlContent => {
                    let range = vscode.Range.create(
...
export class JSONCompletionItemProvider implements CompletionItemProvider {
    ...
    public provideCompletionItems(document: LinesTextDocument, position: Position, token: CancellationToken): Thenable<CompletionList> {
        ...
        if (location.isAtPropertyKey) {
            ...
            collectPromise = this.jsonContribution.collectPropertySuggestions(vscode.Uri.parse(document.uri).fsPath, location, currentWord, addValue, isLast, collector);
        } else {
            if (location.path.length === 0) {
                collectPromise = this.jsonContribution.collectDefaultSuggestions(vscode.Uri.parse(document.uri).fsPath, collector);
            } else {
                collectPromise = this.jsonContribution.collectValueSuggestions(vscode.Uri.parse(document.uri).fsPath, location, collector);
            }
        }
        if (collectPromise) {
    ...
    private getCurrentWord(document: LinesTextDocument, position: Position) {
